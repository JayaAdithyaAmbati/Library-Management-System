    ...
    useEffect(() => {
        if (token) {
            axios.get('http://localhost:5000/myprofile', {
                ...
                .then(res => {
                    setData(res.data);
                    axios.get(`http://localhost:5000/updateHistory1/${res.data._id}`)
                        .then(response => {
        ...
        <div>
            ...
            <div style={{ width: "100%", marginLeft: "auto", marginRight: "auto",  marginTop: "90px" }}>
                <div style={{ backgroundColor: "white", color: "black", borderRadius: "7px", display: "flex", flexWrap: "wrap", marginTop: "6%", marginBottom: "10px" }}>
                    <img src={process.env.PUBLIC_URL + '/reader.png'} alt="not found" height={50} style={{ marginLeft: "10px" }} />
                    <h1>Books Used so far</h1>
                </div>
            <Container fluid>
                {history.length > 0 ? (
                    <Row>
